...
HotSpotApp.controller('CollectionController', ['$scope','$routeParams','$rootScope', '$http', function ( $scope, $routeParams, $rootScope , $http) { 
  $scope.collection = $rootScope.Collections.find( function(el) { return el.id == $routeParams.collectionId });
  ...
  function refreshHotspotCollection(){
    ...
    .success( function( data ) {
      if ( data == "null" ) { $scope.hotspot_collection = {}; } 
      else { 
  ...
  function refreshInterfaces(){
    ...
    .success( function( data ) {
      $scope.interfaces = {};
      if ( data != "null" ) {
        $scope.interfaces.slider = data.find( function(el) { return el.title == 'slider' } ); 
        $scope.interfaces.trend = data.find( function(el) { return el.title == 'trend' } ); 
        $scope.interfaces.gallery = data.find( function(el) { return el.title == 'gallery' } ); 
      }
    });
  }
}]); 
...
function showHotSpot( elem ){
  if ( elem ) {
    ...
    $('.hotspot-body').css({'top': elem.offset().top, 'left': elem.offset().left + 16, 'position':'absolute', 'border':'1px solid black', 'padding':'5px'});
    $('.hotspot-body').show();
  } else {
    $('.hotspot-body').remove();
  }
